...
public class RobotContentMemoryAdapter: RobotContentAdapter {
    var items: [String:RobotContent] = [:]

    public func findAll(onCompletion: @escaping ([RobotContent], Error?) -> Void) {
        onCompletion(items.map { $1 }, nil)
    }

    public func create(_ model: RobotContent, onCompletion: @escaping (RobotContent?, Error?) -> Void) {
    ...
    public func findOne(_ maybeID: String?, onCompletion: @escaping (RobotContent?, Error?) -> Void) {
    ...
    public func update(_ maybeID: String?, with model: RobotContent, onCompletion: @escaping (RobotContent?, Error?) -> Void) {
    ...
    public func delete(_ maybeID: String?, onCompletion: @escaping (RobotContent?, Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
