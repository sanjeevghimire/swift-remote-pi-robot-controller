...
class RouteTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            ...
            Log.error("Couldn't start Application test server. Exiting test.")
        }
        
        Kitura.addHTTPServer(onPort: Application.port, with: Application.router)
        Kitura.start()
...
private extension URLRequest {
    ...
    init?(forTestWithMethod method: String, route: String = "", body: Data? = nil) {
        if let url = URL(string: "http://127.0.0.1:\(Application.port)/" + route){
            self.init(url: url)
