...
class RemoteRobotUtility{
    
    internal static let basePath = getBasePath()
    
    private static func getBasePath() -> String {
        if let path = Bundle.main.path(forResource: "RemoteRobotConfig", ofType: "plist"), let credentials = NSDictionary(contentsOfFile: path) as? [String: AnyObject] {
            if let basePath = credentials["RemoteRobotHost"] as! String? , !basePath.isEmpty {
                return basePath
            }
        }
...
extension RemoteRobotUtility {
    ...
    internal static func convertToData<T: Mappable>(_ input: T?) -> Data? {
        ...
        return try? JSONSerialization.data(withJSONObject: value.toJSON(), options: [])
    }
    ...
    internal static func convertToData<T: Mappable>(_ input: Array<T>?) -> Data? {
        guard let value = input else { return nil }
        let inputJSON: [[String: Any]] = value.map { (mappable: T) -> [String: Any] in
            mappable.toJSON()
        }
